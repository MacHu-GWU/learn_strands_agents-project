...
```
learn_strands_agents-project/
├── learn_strands_agents/
│   ├── __init__.py
│   ├── auth/
│   │   ├── __init__.py
...
### learn_strands_agents/auth/users.py
...
### learn_strands_agents/auth/permissions.py
...
### learn_strands_agents/database/models.py
...
### learn_strands_agents/database/connection.py
...
def process_user():
    ...
    Creates a :class:`~learn_strands_agents.auth.users.User` instance and validates
    permissions through :class:`~learn_strands_agents.auth.permissions.Role`.
    """
    pass
```
...
def update_profile():
    ...
    Uses :meth:`User.get_profile <learn_strands_agents.auth.users.User.get_profile>` 
...
def login_user():
    ...
    Calls :func:`~learn_strands_agents.auth.users.authenticate` to verify credentials
    and :func:`~learn_strands_agents.auth.permissions.check_permission` for authorization.
    """
    pass
```
...
def setup_auth():
    ...
    Configures components from :mod:`learn_strands_agents.auth.users` and 
    :mod:`learn_strands_agents.auth.permissions` modules.
    """
    pass
```

**Rendered as**: Configures components from `learn_strands_agents.auth.users` and `learn_strands_agents.auth.permissions` modules.

### 2. Referencing Elements Within the Same Module

Use short paths when referencing elements in the same Python file:

#### Example: learn_strands_agents/auth/users.py
...
:ref:`getting-started` guide, then review the :class:`~learn_strands_agents.database.models.BaseModel`
class documentation.

For connection management, see :func:`~learn_strands_agents.database.connection.connect`
...
:func:`~learn_strands_agents.database.connection.connect`.
...
class BaseModel:
    ...
    through :func:`~learn_strands_agents.database.connection.connect`.
    
    Integration with authentication is handled through 
    :class:`~learn_strands_agents.auth.users.User` instances. User permissions are
    validated using :func:`~learn_strands_agents.auth.permissions.check_permission`.
    ...
    def save(self) -> None:
        ...
        Uses the connection established by :func:`~learn_strands_agents.database.connection.connect`.
        Requires authentication through :func:`~learn_strands_agents.auth.users.authenticate`.
        """
        pass
    ...
    def find_by_id(self, record_id: int) -> Optional[dict]:
        ...
        .. seealso::
            ...
            :func:`~learn_strands_agents.database.connection.Database.query` for custom queries.
        """
        pass
```
...
class UserManager:
    ...
    **Core Components:**
    ...
    - User creation: :class:`~learn_strands_agents.auth.users.User`
    - Authentication: :func:`~learn_strands_agents.auth.users.authenticate`
    - Authorization: :func:`~learn_strands_agents.auth.permissions.check_permission`
    - Data persistence: :class:`~learn_strands_agents.database.models.BaseModel`
...
:class:`learn_strands_agents.auth.users.User`
